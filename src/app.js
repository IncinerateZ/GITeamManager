    ...
    if (collapsed != null) {
        ...
        if (collapseToggle) {
            ...
            for (let i = 0; i < uncollapsed.childNodes.length; i++) {
                let dat =
                    uncollapsed.childNodes[i].querySelector(
                        '.amt-text',
                    ).innerHTML;
                while (dat.includes(',')) dat = dat.replace(',', '');
                createCard(
                    uncollapsed.childNodes[i].id,
                    dat,
                    uncollapsed.childNodes[i].classList[1],
                );
            }
        }
    } else {
        ...
        let map = {};
        for (let i = 0; i < uncollapsed.childNodes.length; i++) {
            if (map[uncollapsed.childNodes[i].id] == undefined)
                map[uncollapsed.childNodes[i].id] = 0;
            let dat =
                uncollapsed.childNodes[i].querySelector('.amt-text').innerHTML;
            while (dat.includes(',')) {
                dat = dat.replace(',', '');
            }
...
var forceAspectRatio = (img, type) => {
    ...
    img.onload = (e) => {
        ...
        if (th < 97) {
            img.parentNode.parentNode.parentNode.lastChild.style.position =
                'relative';
...
var createCard = (name, amount, type) => {
    ...
    let res =
        ...
        '</div>' +
        "<div class='amt-box'>" +
        "<div class='amt-text card-content'>" +
...
window.onload = function () {
    ...
    for (let list in weaponlist) {
        for (let weapon in weaponlist[list]) {
            weapons.innerHTML +=
                "<div class='icon weapons' id='" +
                weaponlist[list][weapon] +
                "'><img class='avatar weap' id='i-" +
                weaponlist[list][weapon] +
                "' alt='" +
                nameextract(weaponlist[list][weapon]) +
                "' src='src/img/webp-weapons/" +
                weaponlist[list][weapon] +
                ".webp'></div>";
            if (weapselected === weaponlist[list][weapon]) {
                var s = document.getElementById(
                    'i-' + weaponlist[list][weapon],
                );
    ...
    for (let weapon in upcomingweapons) {
        weapons.innerHTML +=
            "<div class='icon grey' id='" +
            upcomingweapons[weapon] +
            "'><img class='avatar' id='i-" +
            upcomingweapons[weapon] +
            "' alt='" +
            nameextract(upcomingweapons[weapon]) +
            "' src='src/img/webp-weapons/" +
            upcomingweapons[weapon] +
            ".webp'></div>";
    ...
    startbtn.addEventListener('click', () => {
        ...
        res.forEach((list) => {
            if (list != null) {
                ...
                if (list['mora2'] != undefined)
                    createCard('mora', list['mora2'], 't');
                let mats = list['mat'];
...
var avatarselect = (charicon) => {
    ...
    charname =
        charselected[0].toUpperCase() +
        charselected.substr(1, charselected.length);
...
var weaponselect = (weapicon) => {
    ...
    weapname =
        weapselected[0].toUpperCase() +
        weapselected.substr(1, weapselected.length);
